...
function validateNameDeal() {
    ...
    else if (nameDealValue.length <= 4 || nameDealValue.length >= 100) {
        nameDealError.textContent = 'Tên sản phẩm phải có 4 đến 100 ký tự';
        return false;
    }
    else {
...
//    } else {
//        document.getElementById('dealExpiredatError').innerText = ''; // Xóa thông báo lỗi nếu không có lỗi
...
//    var isImageValid = validateImg(); 
//    var isDateValid =  checkdate();
    
    

//    // Kiểm tra nếu tất cả các trường đều không có lỗi
//    if (isNameValid && isSlotValid && isPriceValid && isRefundPriceValid && isLinkValid && isImageValid && isDateValid) {
...
//    }
//}
//document.getElementById('ImgDeal').addEventListener('change', validateForm);
...
function formatNumber(input) {
    ...
    // Nếu giá trị nhập vào không phải là số, không cập nhật giá trị của input
    if (!isNaN(parseInt(value))) {
        // Cập nhật giá trị của input với định dạng số mới
